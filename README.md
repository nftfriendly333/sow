    ...
    <div class="panel">
      <div class="login-form">
        <div style="text-align:center;margin-bottom:18px;">
          <div style="font-size:11px;color:var(--dim);line-height:1.8;">Connect your Ethereum wallet to access<br>the shared $PvE ledger.</div>
        </div>
        <!-- MetaMask / injected -->
        <button class="btn-gold" id="l-btn-mm" onclick="connectMetaMask()">
          <span style="font-size:18px;margin-right:8px;">🦊</span> CONNECT METAMASK
        </button>
        <!-- WalletConnect -->
        <button class="btn-gold" id="l-btn-wc" onclick="connectWalletConnect()" style="margin-top:10px;background:var(--s2);color:var(--goldb);border:1px solid var(--goldd);">
          <span style="font-size:16px;margin-right:8px;">⬡</span> WALLETCONNECT
        </button>
        <div class="err-msg hidden" id="l-err" style="margin-top:12px;"></div>
        <div style="text-align:center;margin-top:16px;font-size:9px;color:var(--dim);">All users share the same live ledger · Prototype</div>
        <div style="text-align:center;margin-top:6px;font-size:9px;color:var(--dim);">New wallets receive <span style="color:var(--gold);">1000 $PvE locked 🔒</span></div>
      </div>
    </div>
  </div>
</div>
...
async function connectMetaMask(){
  hide('l-err');
  if(!window.ethereum){showErr('MetaMask not detected. Install MetaMask or use WalletConnect.');return;}
  const btn=$('l-btn-mm');
  ...
  try{
    ...
    showErr(e.code===4001?'Connection rejected by user.':'MetaMask error: '+e.message);
    btn.innerHTML='<span style="font-size:18px;margin-right:8px;">🦊</span> CONNECT METAMASK';
    btn.disabled=false;
  }
}
...
async function connectWalletConnect(){
  hide('l-err');
  showErr('WalletConnect support requires the WalletConnect SDK. See setup instructions.');
  // Placeholder: integrate @walletconnect/modal + ethers when deploying
}

async function handleWalletLogin(rawAddr){
  const addr=rawAddr.toLowerCase();
  await loadAll();
  const existing=ST.wallets.find(w=>w.addr.toLowerCase()===addr);
  if(existing){
    // Returning wallet — just log in
    session={username:shortAddr(rawAddr),walletId:existing.id,ethAddr:rawAddr};
    startApp();
  }else{
    // New wallet — register automatically
    const wid='wu'+Date.now();
    const dn=shortAddr(rawAddr);
    const nw={id:wid,name:dn,addr:rawAddr,balance:0,locked:1000,eth:0.1,paused:false,pending:0,tag:'Connected Wallet',color:PALETTE[ST.wallets.length%PALETTE.length],isUser:true};
    ...
    // Persist wallet data keyed by address (no username/password)
...
function doSignOut(){
  ...
  const mm=$('l-btn-mm');
  if(mm){mm.innerHTML='<span style="font-size:18px;margin-right:8px;">🦊</span> CONNECT METAMASK';mm.disabled=false;}
  hide('l-err');
}
